...
function EndGame({ title, accuracy, completed, onAgain, onBack }) {
  return <div className="game-end-card page-enter"><span className="end-game-icon">🎉</span><span className="eyebrow">A lovely effort</span><h2>GREAT JOB, RAMESH JI!</h2><p>{completed ? 'You completed the memory activity!' : 'Every moment of practice helps your mind shine.'}</p><div className="performance-summary"><strong>⭐ Performance Summary</strong><span>You completed: <b>{completed}</b></span><span>Accuracy: <b>{accuracy}%</b></span></div><div className="end-game-actions"><button type="button" onClick={onAgain}>🔄 PLAY AGAIN</button><button type="button" onClick={onBack}>← BACK TO GAMES</button></div><small>{title} is saved for your care circle.</small></div>;
}

function MatchGame({ onBack, difficulty }) {
  ...
  function flipCard(card) {
    ...
    if (nextPicked.length === 2) {
      ...
      if (nextPicked[0].value === nextPicked[1].value) {
        ...
        setMatches((value) => { const next = value + 1; if (next === matchItems.length) { setFinished(true); recordGame('Memory Match', Math.round(((next) / (moves + 1)) * 100), difficulty, `${next} of ${matchItems.length} pairs`); } return next; });
        setPicked([]);
      } else {
  ...
  if (finished) return <EndGame title="Memory Match" accuracy={Math.round((matches / Math.max(moves, 1)) * 100)} completed={`${matches} of ${matchItems.length} pairs`} onAgain={() => window.location.reload()} onBack={onBack} />;
...
function QuizGame({ type, onBack, difficulty }) {
  ...
  const isCorrect = answered === question.answer;
...
export default function MemoryGames({ initialGame }) {
  ...
  if (activeGame === 'match') return <div className="memory-games-page focused-game-page page-enter"><MatchGame difficulty={difficulty} onBack={() => setActiveGame(null)} /></div>;
  if (['who', 'pattern', 'object'].includes(activeGame)) return <div className="memory-games-page focused-game-page page-enter"><QuizGame type={activeGame} difficulty={difficulty} onBack={() => setActiveGame(null)} /></div>;
